...
export class WeaponController {
  ...
  static getChannelRatio(player: Player, weaponId = player.currentWeaponId): number {
    ...
    const channelTime = player.weaponLoadout.slots[player.weaponLoadout.activeSlot].customState.channelTime || 0;
    return Math.max(0, Math.min(1, channelTime / Math.max(0.1, weapon.channelTime ?? 3.2)));
  }
  ...
  static updateRuntime(player: Player, dt: number, fireHeld: boolean): void {
    player.weaponLoadout.slots.forEach((slot, index) => {
      ...
      const strategy = ResourceStrategies[slot.resourceType];
      if (strategy) {
        strategy.update(slot, weaponDef, player, dt, isActive, fireHeld);
      }
    });
  }
  ...
  static fire(
    ...
    // Calculate energyCost before canFire check
    ...
    if (!ignoreCost && strategy) {
      strategy.consume(slot, weapon, player, energyCost);
    }
