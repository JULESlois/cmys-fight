...
export function getStageDifficulty(stage: Pick<StageData, "globalStageIndex" | "chapterIndex"> & Partial<Pick<StageData, "hardMode">>): StageDifficulty {
  ...
  return {
    ...
    speedMultiplier: (1 + Math.min(0.35, progress * 0.025)) * speedScale,
    ...
    eliteChance: globalStageIndex < 3 && !hard ? 0 : Math.min(0.42, 0.08 + (globalStageIndex - 3) * 0.025 + (hard ? 0.12 : 0)),
