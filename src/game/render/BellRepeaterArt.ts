...
export function drawBellRepeaterArtFx(
  ...
  // Independent clapper: gold in normal operation, dark amber below 20% ammo.
  ctx.fillStyle = metrics.lowAmmo ? "#9C6A20" : "#FFF3B0";
  ctx.fillRect(bellX + metrics.clapperOffset, bellY, 2, 3);

  // Sustained fire warms only the compact receiver rim, never to warning red.
  ...
  // Every third successful shot gets one restrained upper half-ring.
  ...
  // Reload keyframes: magazine drop, bell-wheel rollback, magazine insertion.
