...
assert.match(hub, /HUB_ACTIONS: HubAction\[\] = \["start", "hard", "challenge", "refund"\]/);
assert.doesNotMatch(hub, /switchState\("records"\)/);
...
assert.match(records, /drawPixelPanel\(ctx, 14, 53, 151, 172/);
assert.match(records, /drawPixelPanel\(ctx, 172, 53, 134, 172/);
...
assert.match(settings, /const visibleRows = 8/);
assert.match(settings, /const start = Math\.max\(0, Math\.min\(options\.length - visibleRows, this\.selectedIndex - 3\)\)/);
assert.match(settings, /const visible = options\.slice\(start, start \+ visibleRows\)/);
assert.match(settings, /drawMeter/);
...
assert.match(result, /RUN SUMMARY/);
assert.match(result, /FINAL BUILD/);
assert.match(result, /const weaponId = player\.currentWeaponId/);
assert.match(result, /const buildBuffs = \(player\.buffs \?\? \[\]\)\.slice\(0, 6\)/);
assert.match(menuRenderer, /drawMeter\(ctx/);
...
assert.match(hud, /standbyIndex = player\.activeWeaponSlot === 0 \? 1 : 0/);
