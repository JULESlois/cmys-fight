...
assert.equal(promptInput.getPrompt("subWeapon"), "LT");
assert.equal(promptInput.getPrompt("crush"), "SELECT");
...
assert.equal(promptInput.getPrompt("subWeapon"), "U", "sub-weapon defaults to the U key");
assert.equal(promptInput.getPrompt("crush"), "O", "item crush defaults to the O key");
...
// Castlevania-layer actions: U/LT throws the sub-weapon, O/SELECT triggers Item Crush.
const castlevaniaInput = new Input();
windowTarget.dispatch("keydown", { key: "u", preventDefault() {} });
assert.equal(castlevaniaInput.wasActionPressed("subWeapon"), true, "default U key throws the sub-weapon");
assert.equal(castlevaniaInput.wasActionPressed("crush"), false);
castlevaniaInput.update();
windowTarget.dispatch("keyup", { key: "u", preventDefault() {} });
windowTarget.dispatch("keydown", { key: "o", preventDefault() {} });
assert.equal(castlevaniaInput.wasActionPressed("crush"), true, "default O key triggers Item Crush");
assert.equal(castlevaniaInput.wasActionPressed("subWeapon"), false);
castlevaniaInput.update();
windowTarget.dispatch("keyup", { key: "o", preventDefault() {} });
const subWeaponPad = createPad(false);
subWeaponPad.buttons[6] = { pressed: true, value: 1 };
gamepads = [subWeaponPad];
castlevaniaInput.beginFrame();
assert.equal(castlevaniaInput.wasActionPressed("subWeapon"), true, "gamepad LT throws the sub-weapon");
assert.equal(castlevaniaInput.getPrompt("subWeapon"), "LT");
castlevaniaInput.update();
gamepads = [createPad(false)];
castlevaniaInput.beginFrame();
castlevaniaInput.update();
const crushPad = createPad(false);
crushPad.buttons[8] = { pressed: true, value: 1 };
gamepads = [crushPad];
castlevaniaInput.beginFrame();
assert.equal(castlevaniaInput.wasActionPressed("crush"), true, "gamepad SELECT triggers Item Crush");
assert.equal(castlevaniaInput.getPrompt("crush"), "SELECT");
castlevaniaInput.update();
gamepads = [];
castlevaniaInput.cleanup();
