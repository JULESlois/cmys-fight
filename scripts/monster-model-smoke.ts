...
assert.match(source, /root_lancer[\s\S]*three-step charge read/, "root lancer exposes a staged sniper charge");
assert.match(source, /root_lancer[\s\S]*Body, shaft, leaf head and sap conduit/, "root lancer separates its rifle material layers");
assert.match(source, /root_lancer[\s\S]*aimStage >= 3[\s\S]*rgba\(205,230,111,0\.35\)/, "root lancer reserves a narrow lock line for the final charge stage");
assert.match(source, /root_lancer[\s\S]*const recoil = state === "attack" \? \[0, 0, -3, -1\]/, "root lancer rifle recoils three pixels without moving its body");
...
for (const theme of ["Forest", "Dungeon", "Snow", "Lava"]) {
  assert.match(doorRenderer, new RegExp(`function drawTop${theme}Door`), `${theme} door has an authored top-door model`);
  assert.match(doorRenderer, new RegExp(`function drawBottom${theme}Door`), `${theme} door has an authored bottom-door model`);
}
assert.doesNotMatch(
  roomRenderer,
