...
assert.equal(migrated.keyBindings.subWeapon, "u", "legacy saves gain the sub-weapon binding");
assert.equal(migrated.keyBindings.crush, "o", "legacy saves gain the item-crush binding");
assert.equal(defaults.keyBindings.subWeapon, "u");
assert.equal(defaults.keyBindings.crush, "o");
...
assert.match(canvasSource, /const nextTouchLabelMode = settings\.touchLabelMode/);
assert.match(canvasSource, /setTouchLabelMode\(nextTouchLabelMode\)/);
assert.match(canvasSource, /buildTouchLabels\(nextTouchLabelMode, settings\.keyBindings\)/);
